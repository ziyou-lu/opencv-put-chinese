        ...
        for (auto j = 0; j < cols; ++j)
        {
            ...
            if (slot->bitmap.buffer[off] & (0x80 >> (j % 8)))
            {
                ...
                if (r >= 0 && r < img.rows && c >= 0 && c < img.cols)
                {
                    ...
                    switch (img.channels()) {
                        case 2: {
                            cv::Vec2b scalar = img.at<cv::Vec2b>(cv::Point(c, r));
                            ...
                            img.at<cv::Vec2b>(cv::Point(c, r)) = cv::Vec2b(scalar[0], scalar[1]);
                            break;
                        }
                        case 3: {
                        ...
                        case 4: {
                            ...
                            img.at<cv::Vec4b>(cv::Point(c, r)) = cv::Vec4b(scalar[0], scalar[1], scalar[2]);
                            break;
                        }
                        default:
                            std::cerr << "not support channel num" << std::endl;
                            break;
                    }
